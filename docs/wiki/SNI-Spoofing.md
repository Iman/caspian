...
Replacing the config clears the previous spoof name.
Selecting another entry or refreshing a subscription preserves it.

## State compatibility

This branch writes state schema version 4.
...
Windows x64 needs the WinDivert files included in the local installer build.
Windows ARM64 cannot use this feature.
Linux needs packet-socket privileges; macOS needs access to a BPF device on an Ethernet-style interface.
...
See [third-party credits](https://github.com/Iman/caspian/blob/feature/sni/docs/THIRD-PARTY.md) and the [source review](https://github.com/Iman/caspian/blob/feature/sni/docs/THIRD-PARTY.md).
...
Read [SNI setup and limitations](https://github.com/Iman/caspian/blob/feature/sni/docs/SNI.md) before enabling it.
