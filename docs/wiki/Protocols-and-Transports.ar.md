...
| المستندات الملصقة | Clash and Clash.Meta YAML، إعداد xray JSON كامل أو مصفوفة JSON منها (إعداد "Custom" من v2rayN أو v2rayNG، أو اشتراك BPB يُقدَّم لـ xray)، قائمة الروابط واحدة لكل سطر، اشتراك base64 blob | عنوان URL للاشتراك، ومستند Clash مغلف بقاعدة 64، وإعداد xray JSON يتصل الوكيل فيه عبر outbound آخر، ونص يكون سطره الأول عبارة عن تعليق |
...
يُقرأ إعداد xray JSON الكامل من أجل الوكيل فقط ولا شيء غيره. كل كائن هو إدخال واحد في
القائمة، ويُسمّى بحقل remarks الخاص به. من كل كائن يؤخذ الـ outbound الموسوم proxy، وإلا
فأول outbound ببروتوكول وكيل؛ وتُحفظ إعدادات stream الخاصة به كما كُتبت، بما في ذلك finalmask
وخيارات المقبس وترويسة HTTP لـ TCP وبصمة TLS وALPN واسم الخادم. أما inbounds والتوجيه وDNS
وpolicy وبقية outbounds في الإعداد فيتم تجاهلها، لأن الصندوق يقررها بنفسه. الإعداد الذي لا
يحتوي على خادم وكيل، مثل إدخال BPB يكون وكيله outbound من نوع freedom، يُرفض. وكذلك يُرفض
الوكيل المتسلسل عبر outbound آخر باستخدام dialer proxy أو proxy settings: ضع الجزء (fragment)
في finalmask الخاص بالوكيل نفسه بدلًا من ذلك، وهكذا يكتبه BPB أصلًا.
