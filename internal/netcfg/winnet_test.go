...
func TestWindowsPlan_TunnelsTheWholeHostFailClosed(t *testing.T) {
	...
	if !strings.Contains(pre[1].Do.Stdin, `"forward":"normal"`) || !strings.Contains(pre[1].Do.Stdin, `"tun":"xray0"`) {
		t.Fatalf("filter set = %s", pre[1].Do.Stdin)
	}
	// The uplink's forwarding flag was read as off, so the inverse says off.
	if CommandLine(pre[2].Undo) != "iphlpapi iface set xray0 forwarding off" {
		// xray0 did not exist at detection time, so its knob was not read: no inverse.
		if !pre[2].Undo.IsZero() {
			t.Fatalf("forwarding inverse = %s", pre[2].Undo)
		}
	}
